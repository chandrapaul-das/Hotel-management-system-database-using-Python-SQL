...
def staff_menu():
    ...
    if ans == "1":
        ...
        for i in range(3):
            ...
            if i == 1:
                c.execute(f"select ID,persons,duration,code from customer where uid = {cid}")
                s2 = clean(c.fetchall())
                if s2 != []:
                    person=0
                    s3 = list(map(str, s2.split()))
                    if int(s3[1]) > 3:
                        person += (int(s3[1])-3)*500
                    if s3[0][0] == 'G':
                        if s3[3] == '0':
                            b1 = 2300*int(s3[2]) + person*int(s3[2])
                            print(f"The bill is: {b1} INR")
                        if s3[3] == 'DAYGUEST':
                            print(f"The bill is: 3000 INR")
                    if s3[0][0] == 'D':
                        if s3[3] == '0':
                            b2 = 2900*int(s3[2]) + person*int(s3[2])
                            print(f"The bill is: {b2} INR")
                        if s3[3] == 'DELUXE30':
                            b2 = (2900*int(s3[2]) + person*int(s3[2]))*0.7
                            print(f"The bill is: {b2} INR")
                    if s3[0][0] == 'P':
                        if s3[3] == '0':
                            b3 = 3800*int(s3[2]) + person*int(s3[2])
                            print(f"The bill is: {b3} INR")
                        if s3[3] == 'DAYPREMIER':
                            print("The bill is: 3700 INR")
                        if s3[3] == 'DPREMIER2800':
            ...
            if i == 2:
                c.execute(f"select * from picnic_book where uid = {cid}")
                s2 = clean(c.fetchall())
                if s2 != []:
